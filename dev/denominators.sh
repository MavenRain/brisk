...
#   zsh /Users/oobi/Documents/brisk/dev/denominators.sh
#
# Method, fixed by D-M0-7 (M0-PLAN.md:29 and :342):
...
#   3.  Copy the files to $TMPDIR/brisk-denom-$$ and order the copy with
...
#
# The NUMSHA line is a fresh sha256 of examples/m0-spine.bk taken in this
# run, so the numerator pin is checked by the file and not by its own
# record (M0-PLAN.md:253).
#
# NEW.  There is no kanon counterpart;  kanon froze its denominator once.
# See dev/PROVENANCE.md.
...
SPINE=$ROOT/examples/m0-spine.bk
...
# The bench line carries the min and the max, which the build log records.
# It leaves stdout clean:  a caller that wants it names a file.
[[ -n ${BRISK_DENOM_BENCH:-} ]] && print -r -- "$bench" > $BRISK_DENOM_BENCH
